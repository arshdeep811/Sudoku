...
var medium = [ "","","","",6,"","",7,"",
   ...
   var mediumAns = [ 5,3,9,1,6,8,4,7,2,
                     1,6,8,7,4,2,3,5,9,
                     7,2,4,9,5,3,8,1,6, 
                     6,7,3,8,1,4,9,2,5,
                     2,9,1,5,3,7,6,4,8,
...
function validate() {
  ...
  let l = 1;
    ...
    for (let i = 0; i < 9; i++) {
      if (userAns[i] != "")
         sum += parseInt(userAns[i]);
      for (let j = i + 9; j < 81; j += 9) {
        if (userAns[j] != "") 
          sum += parseInt(userAns[j]);
      }
    }
    if (sum != 405) {
      l = 0;
...
function removeWrong(id) {
  getId(id).classList.remove("wrong");
}
...
function highlightSameNum(id) {
    ...
    var userAns = getUserAnswers();
    var num = getId(id).value;
    for(let i=0; i<input.length; i++)
    {
        input[i].classList.remove("highlight-same");
    }
    ...
    for(let i=0; i<userAns.length; i++)
    {
           ...
           if(userAns[i] == num)
           { 
              input[i].classList.add("highlight-same");
           }
    }
}
